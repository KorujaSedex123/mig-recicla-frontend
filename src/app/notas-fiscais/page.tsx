...
export default function NovaNotaFiscal() {
  ...
  const [formData, setFormData] = useState({
    ...
    dataEmissao: "",
    clienteId: "",
    // --- Novos campos para a saída das bags ---
    quantidadeBags: "",
    prazoLimite: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Primeiro, criamos a Nota Fiscal
      const payloadNF = {
      ...
      const notaFiscalCriada = await resNF.json(); // Pegamos o ID da NF que acabou de ser gerada no banco

      // 2. Em seguida, criamos o registro de saída das Embalagens (PENDENTE)
      const payloadBags = {
        quantidadeBags: Number(formData.quantidadeBags),
        status: "PENDENTE",
        prazoLimite: formData.prazoLimite,
        notaFiscal: { id: notaFiscalCriada.id }, // Amarramos à NF recém-criada!
      };

      const resBags = await fetch("http://localhost:8080/api/embalagens-retorno", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payloadBags),
      });
      ...
      setMensagem({ texto: "Ocorreu um erro na operação. Verifique os dados.", tipo: "erro" });
    } finally {
      setLoading(false);
    }
  };
        ...
        <header className="mb-8 flex justify-between items-center border-b pb-4">
          <div>
            ...
            <p className="text-gray-600 mt-2 text-sm">
              Registre a saída dos produtos e defina o prazo para retorno das embalagens.
            </p>
          </div>
          <Link
            href="/"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                />
              </div>
            </div>
